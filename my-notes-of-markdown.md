...
关于表格。亲测`github`表格显示默认左对齐，唯一例外是当表头（第一行）比表格内容短，则该列表头居中。在`github`里可以使用`:-`、`:-:`、`-:`分别使该列左对齐、居中、右对齐，但是在`jupyter notebook`里用这个语法调整不了对齐方式，默认右对齐。所以涉及表格的一律不调整对齐方式，按各平台默认方式识别即可。
