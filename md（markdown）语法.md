...
在展示原语法和效果的时候，会用到大量代码块，由于频繁使用` ``` `会导致：1. 平台编译前文章格式的混乱，不利于我审阅原文和修改；2. 平台错误识别代码块，把原本在一起的代码块分成几个，有的内容甚至没有按原本期望的代码块形式展示。因此，把演示语法部分的代码都用tab分块，代码块前接`#### 语法`，后接`#### 效果`并实际演示。如果一个子级主题中有好几个代码示例或者完成示例后有其他说明，则在每例代码效果展示完成后用`***`分隔。
...
我想将`这里`高亮显示。
***
特别地，如果想将`` ` ``本身高亮显示，在`` ` ``前后都加上` `` `并用空格隔开`` ` ``和` `` `。
#### 语法
    如果有`` ` ``就这么用。
#### 效果
如果有`` ` ``就这么用。
...
## 引用
`> `会自动识别并引用整个段落，也就是即使用2个空格加回车完成了段内换行，仍会继续引用至本段结束。
#### 语法
    > 从这里开始是引用。  
    这里也是。
#### 效果
...
这里也是。
***
还可以嵌套引用或者在引用内使用标题、列表（markdown官网上说还可以嵌套代码块，但是我暂时没研究出来怎么实现，）。
#### 语法
    > 第一层引用
    >> 第二层
    >>> 第三层
...
### 参考式
#### 语法
    [北理工]:http://www.bit.edu.cn/images/2013zzgb/logo.jpg
    第一次用![替换文本][北理工]，第二次用![替换文本][北理工]，...，多次用![替换文本][北理工]。
#### 效果
[北理工]:http://www.bit.edu.cn/images/2013zzgb/logo.jpg
第一次用![替换文本][北理工]，第二次用![替换文本][北理工]，...，多次用![替换文本][北理工]。
***
